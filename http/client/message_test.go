...
func testMessages(t *testing.T, env *env, tk testKeys) {
	...
	require.False(t, msgs.Truncated)
	out1, err := api.DecryptMessage(msgs.Messages[3], saltpack.NewKeyring(channel))
	require.NoError(t, err)
	require.Equal(t, msg1.Text, out1.Text)
	require.Equal(t, alice.ID(), out1.Sender)
	out2, err := api.DecryptMessage(msgs.Messages[4], saltpack.NewKeyring(channel))
	require.NoError(t, err)
	...
	out3, err := api.DecryptMessage(msgs.Messages[0], saltpack.NewKeyring(channel))
	require.NoError(t, err)
	...
	out1, err = api.DecryptMessage(msgs.Messages[0], saltpack.NewKeyring(channel))
	require.NoError(t, err)
	require.Equal(t, msg3.Text, out1.Text)
	out2, err = api.DecryptMessage(msgs.Messages[1], saltpack.NewKeyring(channel))
	require.NoError(t, err)
	require.Equal(t, msg2.Text, out2.Text)
	out3, err = api.DecryptMessage(msgs.Messages[2], saltpack.NewKeyring(channel))
	require.NoError(t, err)
