...
func TestMessages(t *testing.T) {
	...
	// POST /messages/:kid (channel=wormhole)
	req, err = api.NewRequest("POST", keys.Path("messages", charlie.ID())+"?channel=wormhole", bytes.NewReader([]byte("test1")), clock.Now(), charlie)
	require.NoError(t, err)
	code, _, _ = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)
	req, err = api.NewRequest("POST", keys.Path("messages", charlie.ID())+"?channel=wormhole", bytes.NewReader([]byte("test2")), clock.Now(), charlie)
	require.NoError(t, err)
	code, _, _ = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)
	req, err = api.NewRequest("POST", keys.Path("messages", charlie.ID())+"?channel=wormhole", bytes.NewReader([]byte("test3")), clock.Now(), charlie)
	require.NoError(t, err)
	code, _, _ = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)

	// GET /messages/:kid (channel=wormhole)
	req, err = api.NewRequest("GET", keys.Path("messages", charlie.ID())+"?channel=wormhole&dir=asc", nil, clock.Now(), charlie)
	require.NoError(t, err)
	code, _, body = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)
	...
	require.NoError(t, err)
	require.Equal(t, keys.ID("kex1a4yj333g68pvd6hfqvufqkv4vy54jfe6t33ljd3kc9rpfty8xlgs2u3qxr"), resp3.KID)
	...
	// GET /messages/:kid (channel=wormhole, descending, limit=2)
	req, err = api.NewRequest("GET", keys.Path("messages", charlie.ID())+"?channel=wormhole&direction=desc&limit=2", nil, clock.Now(), charlie)
	require.NoError(t, err)
	code, _, body = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)
