...
func TestShare(t *testing.T) {
	...
	// PUT /share/:kid (1m)
	req, err := api.NewRequest("PUT", docs.Path("share", key.ID())+"?expire=1m", bytes.NewReader([]byte("hi")), env.clock.Now(), key)
	...
	req, err = api.NewRequest("PUT", docs.Path("share", key.ID())+"?expire=1ms", bytes.NewReader([]byte("hi")), env.clock.Now(), key)
	require.NoError(t, err)
	code, _, body = srv.Serve(req)
	require.Equal(t, http.StatusOK, code)
	require.Equal(t, `{}`, body)
	...
	// PUT /share/:kid (30m)
	req, err = api.NewRequest("PUT", docs.Path("share", key.ID())+"?expire=30m", bytes.NewReader([]byte("hi")), env.clock.Now(), key)
	require.NoError(t, err)
	code, _, body = srv.Serve(req)
	...
	req, err = api.NewRequest("PUT", docs.Path("share", key.ID())+"?expire=30m", bytes.NewReader([]byte("hi")), env.clock.Now(), key2)
	require.NoError(t, err)
	code, _, body = srv.Serve(req)
	require.Equal(t, http.StatusForbidden, code)
	require.Equal(t, `{"error":{"code":403,"message":"invalid kid"}}`, body)
